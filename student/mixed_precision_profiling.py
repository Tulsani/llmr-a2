'''
Benchmarking script with NVTX profiling + optional BF16 mixed precision
'''
...
def get_autocast_ctx(args):
    """
    Returns BF16 autocast context when --mixed_precision is set,
    otherwise a no-op nullcontext so the rest of the code is identical.
    """
    ...
    if args.mode == "forward":
        ...
        def forward_pass():
            with nvtx.range("forward"):
                # autocast applies to inference too — matmuls run in BF16
                with autocast_ctx:
                    _ = model(x)
